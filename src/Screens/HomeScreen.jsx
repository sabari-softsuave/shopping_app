...
export default function HomeScreen() {
  ...
  // Helper to map API product structure to app's ProductCard expectations
  ...
  const fetchProductData = async (query, limit, setter, isViewAll) => {
    try {
      // If View All is clicked, remove limit (or set a high limit) by not appending &limit=5
      const url = isViewAll
      ...
      setter(data.products.map(mapProduct));
    } catch (error) {
  ...
  useEffect(() => {
    ...
    fetchProductData('dress', 5, setDealsData, false);
    fetchProductData('phone', 5, setTrendingData, false);
  }, []);
  ...
  return (
    <ScrollView contentContainerStyle={{ paddingBottom: 100, }}>
      ...
      <Section title="Deal of the Day">
        <HorizontalProducts data={dealsData} />
      </Section>
      ...
      <Section title="Trending Products">
        <HorizontalProducts data={trendingData} />
      </Section>
